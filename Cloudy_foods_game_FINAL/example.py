...
class Food(pygame.sprite.Sprite):
    def __init__(self, xpos, ypos, speed) :
        ...
        food_images = ('food1.png','food2.png','food3.png','food4.png','food5.png','food6.png','food7.png','food8.png','food9.png','food10.png',\
                       'food11.png','food14.png','food15.png','food16.png','food17.png','food18.png','food19.png','food20.png', \
                       'food21.png','food22.png','food23.png','food24.png','food25.png','food26.png','food27.png','food28.png','food29.png','food30.png', \
                       'food31.png','food32.png','food33.png','food34.png','food35.png','food36.png','food37.png','food38.png','food39.png','food40.png', \
                       'food41.png','food42.png')
        
        self.image = pygame.image.load(random.choice(food_images))
        self.rect = self.image.get_rect()
        self.rect.x = xpos
        self.rect.y = ypos
        self.speed = speed
...
def game_loop() :
    ...
    while not done :
        ...
        # 음식을 맞추면 음식이 점점 많이 등장하게
        occur_of_foods = 1 + int(shot_count / 50)

        # 음식 스피드가 점점 올라가게                
        min_food_speed = 1 + int(shot_count / 50)
        max_food_speed = 1 + int(shot_count / 20)
        ...
        if random.randint(1, occur_prob) ==1:
            for i in range(occur_of_foods):                                                     # 운석생성
                speed = random.randint(min_food_speed, max_food_speed)
                food = Food(random.randint(0,WINDOW_WIDTH - 30), 0, speed)                     #운석이 윈도우 끝에 있으면 안보이니까 -30해서 이동시킨 것.
                foods.add(food)
        ...
        for missile in missiles:
            food = missile.collide(foods)                                                      #미사일을 운석이랑 다 충돌했는지 비교해봐서 충돌했으면 반환을 해주는 것
            if food:
                missile.kill()
                food.kill()
                occur_explosion(screen, food.rect.x, food.rect.y)
                shot_count += 1

        for laser in lasers:
            food = laser.collide(foods)                                                        #미사일을 운석이랑 다 충돌했는지 비교해봐서 충돌했으면 반환을 해주는 것
            if food:
                food.kill()
                occur_explosion(screen, food.rect.x, food.rect.y)
                shot_count += 1
        ...
        # 3번 음식이 떨어지면 게임 끝나도록 설정 (목숨3개)
        if fighter.collide(foods) or count_missed >= 3:
            pygame.mixer_music.stop()
...
def main():
    ...
    pygame.display.set_caption('PyShooting')                            #제목주기
